...
ShPtr<Expression> LLVMInstructionConverter::visitInstruction(llvm::Instruction &inst) {
	llvm::UnaryOperator *ui = llvm::dyn_cast<llvm::UnaryOperator>(&inst);
	llvm::outs() << inst.getOpcode() << " vs " << llvm::Instruction::FNeg;
	// 2023年8月24日 好像这个llvm有bug，cast不过去。必须强制转换。
	if (inst.getOpcode() == llvm::Instruction::FNeg) {
		ui = (llvm::UnaryOperator *)(&inst);
	}
	if (ui) {
		switch (ui->getOpcode()) {
		case llvm::UnaryInstruction::FNeg:
			return NegOpExpr::create(getConverter()->convertValueToExpression(inst.getOperand(0)));
		case llvm::Instruction::UnaryOpsEnd: break;
		}
	}
